...
public:
    ...
    {
        // Tiny built-in 5x7 bitmap font for debug/labels without platform text APIs.
        auto glyphFor = [](char c) -> const uint8_t* {
        ...
        for (char ch : text) {
            ...
            for (int row = 0; row < 7; ++row) {
                ...
                for (int col = 0; col < 5; ++col) {
                    ...
                    if ((bits & mask) != 0u) {
                        drawRectangle({cursorX + col * px, y + row * px, px, px}, color);
                    }
                }
            }
            cursorX += advance;
